...
# --- Load Model + Scaler ---
try:
    scaler = joblib.load("D:\\bike_share\\scaler.pkl")
    model = MLPModel(5, 5)
...
def generate_bike_data(n=20):
    ...
    for i in range(n):
        ...
        if model_loaded:
            ...
            # Use dummy data if model not loaded
...
with st.sidebar:
    ...
    if not model_loaded:
        st.warning("Model not loaded. Using demo mode with random predictions.")
    ...
    if st.button("🔮 Predict Failure Risks"):
        if model_loaded:
            ...
            # Generate random predictions if model not loaded
            probs = [random.random() for _ in range(5)]
            probs = [p/max(probs) for p in probs]  # Normalize
